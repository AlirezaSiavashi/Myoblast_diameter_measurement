"""
Train MHNetDS: gap-tolerant Dynamic-Snake U-Net for myotube instance embedding.
...
Run:  C:/mlenv/Scripts/python.exe src/train_ds.py
Out:  output/models/mem_net_ds.pt
"""
...
HELD = {22, 24, 25, 30, 34}
NITER = int(os.environ.get("NITER", "2500"))
...
def build():
    ...
    for pic in [k for k in MAP if k not in HELD]:
        ...
        data.append((mhc, dapi, inst))
    return data


def augment(mhc, dapi, inst, rng):
    """Geometric + photometric augmentation, plus FIBER-GAP DROPOUT (erase patches ON fibers)."""
    if rng.random() < 0.5: mhc, dapi, inst = mhc[:, ::-1], dapi[:, ::-1], inst[:, ::-1]
    if rng.random() < 0.5: mhc, dapi, inst = mhc[::-1], dapi[::-1], inst[::-1]
    if rng.random() < 0.5:
        mhc, dapi, inst = np.rot90(mhc), np.rot90(dapi), np.rot90(inst)
    mhc = mhc.astype(np.float32).copy(); dapi = dapi.astype(np.float32).copy(); inst = inst.copy()
    ...
    return np.clip(mhc, 0, 255), np.clip(dapi, 0, None), inst


def main():
    data = build()
    print(f"loaded {len(data)} TRAIN fields (held out {sorted(HELD)}), crop {SZ}", flush=True)
    net = load_model(os.path.join(MODELS, "mem_net_cons2.pt"), inp=2, arch="ds", device=DEVICE)
    net.train()
    ...
    def sample():
        mhc, dapi, inst = random.choice(data)
        mhc, dapi, inst = augment(mhc, dapi, inst, rng)
        H, W = inst.shape
        sz = min(SZ, H, W)
        y = rng.integers(0, H - sz + 1); x = rng.integers(0, W - sz + 1)
        sl = (slice(y, y + sz), slice(x, x + sz))
        m, d, i = mhc[sl], dapi[sl], inst[sl]
        pre = prep_mhc(m)
        two = np.stack([pre, norm(d)]).astype(np.float32)
        fg = (i > 0).astype(np.float32)
        valid = np.clip(fg + (pre < 0.06).astype(np.float32), 0, 1)
        return two, fg, valid, i.astype(np.int32)

    def batch(bs=3):
        P = []; Fg = []; V = []; I = []
        for _ in range(bs):
            a, b, c, d = sample(); P.append(a); Fg.append(b); V.append(c); I.append(d)
        t = lambda z: torch.from_numpy(np.stack(z))[:, None].float().to(DEVICE)
        return torch.from_numpy(np.stack(P)).float().to(DEVICE), t(Fg), t(V), t(I)

    t0 = time.time()
    for it in range(NITER):
        P, Fg, V, I = batch()
        sem, emb, ori, mem = net(P); prob = torch.sigmoid(sem)
        bce = (F.binary_cross_entropy_with_logits(sem, Fg, reduction="none") * V).sum() / (V.sum() + 1e-6)
        loss = bce + 1.0 * dice(prob, Fg, V) + 2.5 * disc(emb, I, dv=0.3, dd=1.5)
        opt.zero_grad(); loss.backward(); opt.step(); sched.step()
        if it % 250 == 0:
            print(f"  it{it}: loss={loss.item():.3f} [{time.time()-t0:.0f}s]", flush=True)
    torch.save(net.state_dict(), os.path.join(MODELS, "mem_net_ds.pt"))
    print(f"DONE {(time.time()-t0)/60:.1f} min -> output/models/mem_net_ds.pt", flush=True)
