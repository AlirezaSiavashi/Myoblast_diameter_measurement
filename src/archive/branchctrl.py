...
def instances(mhc,dapi,PRUNE,CONNECT):
    ...
    if PRUNE: sk=prune_spurs(sk,9)                                        # FIX 1: remove spurs -> real junctions only
    nb=convolve(sk.astype(int),K8,mode='constant')
    ...
    for i in ids:
        ...
        for j in np.unique(frag[di&(frag!=i)&(frag>0)]):
            ...
            od=np.dot(mO[i],mO[j])
            if CONNECT:
                merge = od>-0.35                                          # FIX 2: connectivity-first; refuse only true crossings
            else:
                merge = np.linalg.norm(mE[i]-mE[j])<1.0 and od>0
            if merge: par[find(i)]=find(j)
...
def caliber(instmask,full_dist,R=7):
    ...
    dvals=full_dist[ys,xs]; i=int(dvals.argmax()); y,x=int(ys[i]),int(xs[i]); w=2*full_dist[y,x]
...
def measure(inst,dist,mhcp):
    ...
    return ws,cals
# --- quantitative: compare OLD vs NEW instance logic ---
for exp in ["E40 hpMb L","E44 hpMb G Klara"]:
    for tag,PR,CN in [("OLD (no prune, embed-merge)",False,False),("NEW (prune + connectivity)",True,True)]:
        auto=[]; km=[]; nn_=[]
        for rf,fld in fields_under(os.path.join(ROOT,exp)):
            ...
            inst,mask,dist=instances(mhc,dapi,PR,CN); ws,_=measure(inst,dist,prep_mhc(mhc))
            if len(ws)<3: continue
            auto.append(np.mean(ws)); km.append(np.mean(gt)); nn_.append(len(ws))
        auto=np.array(auto); km=np.array(km)
        print(f"  {exp[:9]} | {tag:32s}: fibers/field={np.mean(nn_):4.0f} | diam r={pearsonr(auto,km)[0]:.3f} ratio={np.mean(auto/km):.2f} within5={100*np.mean(np.abs(auto-km)<=5):.0f}%",flush=True)
# --- qualitative: E40 field 1 old vs new ---
...
base=np.dstack([disp,np.zeros_like(disp),db])
panels=[]
for tag,PR,CN in [("BEFORE (over-split)",False,False),("AFTER (prune + connectivity)",True,True)]:
    inst,mask,dist=instances(mhc,dapi,PR,CN); ws,cals=measure(inst,dist,prep_mhc(mhc))
    im=Image.fromarray(base.copy()); d=ImageDraw.Draw(im)
    for (pt,perp,w),val in zip(cals,ws):
        y,x=pt; dx,dy=perp[1]*w/2,perp[0]*w/2
        d.line([(x-dx,y-dy),(x+dx,y+dy)],fill=(255,255,0),width=2); d.text((int(x)+3,int(y)-6),f"{val:.0f}",fill=(120,255,120))
    d.text((8,8),f"{tag}: {len(ws)} fibers",fill=(255,255,255)); panels.append(im)
cv=Image.new("RGB",(960*2+10,720),(16,16,16))
cv.paste(panels[0],(0,0)); cv.paste(panels[1],(970,0)); cv.save(OUT+"/branch_control.png")
print("saved -> for_katja/branch_control.png")
