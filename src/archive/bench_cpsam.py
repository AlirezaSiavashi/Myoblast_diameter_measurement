import numpy as np, os, re, glob, csv, time
...
for cond in conds:
    ...
    m=re.search(r'(\d+)\s*d',cond); tp=(m.group(1)+"d") if m else "?"
    ...
    for rf in sorted(glob.glob(os.path.join(cdir,"Result*.txt"))):
        ...
        masks,_,_=model.eval(gaussian(mhc,1.0),diameter=60,flow_threshold=0.4,cellprob_threshold=0.0)
        ws=belly_um(masks)
        if not ws: continue
        rows.append([cond,treat,tp,N,"CellposeSAM",len(ws),round(float(np.mean(ws)),1),round(float(np.mean(gt)),1),len(gt)])
        print(f"{cond:14s} f{N}: n={len(ws):3d} auto={np.mean(ws):5.1f} katja={np.mean(gt):5.1f} [{time.time()-t0:.0f}s]",flush=True)
with open(out+"/e40_cellposesam.csv","w",newline="") as f:
    w=csv.writer(f); w.writerow(["cond","treat","tp","field","model","n_inst","auto_mean","katja_mean","katja_n"]); w.writerows(rows)
print(f"\nDONE {len(rows)} fields in {time.time()-t0:.0f}s -> e40_cellposesam.csv")
