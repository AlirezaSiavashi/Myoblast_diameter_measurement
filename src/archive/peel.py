...
dev="cuda"; PXUM=1.32
MIN_PEEL=int(os.environ.get("MIN_PEEL","45"))   # min path length (px) to still count as a separate fiber
...
def longest_path(S):                      # longest geodesic path across all components of pixel-set S
    if not S: return None
    seen=set(); best=None
    for seed in list(S):
        if seed in seen: continue
        comp=set([seed]); seen.add(seed); q=deque([seed])
        while q:
            u=q.popleft()
            for v in neigh(u,S):
                if v not in comp: comp.add(v); seen.add(v); q.append(v)
        A,_=bfs_far(seed,comp); B,par=bfs_far(A,comp); path=[]; cur=B
...
def peel_centerlines(mask):
    ...
    for rr in regionprops(lab):
        ...
        while True:                                             # peel fibers one by one (longest first)
            path=longest_path(S)
            if path is None or len(path)<MIN_PEEL: break
            ...
            for p in path: S.discard(p)                         # remove this fiber's centerline, then find next
    return out
def nms(cls,dist_thr):                                          # keep spatially-distinct fibers (drop duplicate measurements)
    cls=sorted(cls,key=lambda c:-len(c[3])); kept=[]
    for c in cls:
        y,x=c[1]
        if all((y-ky)**2+(x-kx)**2 > dist_thr**2 for (_,(ky,kx),_,_) in kept): kept.append(c)
    return kept
def parse_um(fp):
    v=[]
    ...
    return v
picks=["E40 hpMb L/scrambled 3d/20x_01","E40 hpMb L/mi133 3d/20x_02","E40 hpMb L/scrambled 5d/20x_01"]
panels=[]
for name in picks:
    fld=ROOT+"/"+name
    mhc=np.array(Image.open(fld+"/Image_CH3.tif").convert("RGB"))[:,:,0]
    dapi=np.array(Image.open(fld+"/Image_CH1.tif").convert("RGB"))[:,:,2]
    cls=nms(peel_centerlines(semseg(mhc)), int(os.environ.get("NMS","45"))); dm=[c[0] for c in cls]
    ...
    im=Image.fromarray(np.dstack([disp,np.zeros_like(disp),db.astype(np.uint8)])); d=ImageDraw.Draw(im)
    for w,(y,x),perp,path in cls:
        ...
        d.ellipse([x-3,y-3,x+3,y+3],fill=(255,255,255)); d.text((int(x)+4,int(y)-7),f"{w:.0f}",fill=(150,255,150))
    d.text((8,8),f"{name.split('/')[-2]}: {len(cls)} fibers | median {np.median(dm):.0f}um",fill=(255,255,255))
    panels.append(im)
    rf=glob.glob(os.path.dirname(fld)+"/Result*.txt"); gt=parse_um(rf[0]) if rf else []
    print(f"{name.split('L/')[-1]:24s} MIN_PEEL={MIN_PEEL} | ours {len(cls):2d} fibers med {np.median(dm):.1f} | Katja {len(gt):2d} med {np.median(gt) if gt else 0:.1f}")
W,H=960,720
cv=Image.new("RGB",(W,H*3+30),(16,16,16)); dd=ImageDraw.Draw(cv)
dd.text((10,6),f"MULTI-FIBER (longest-path peeling, MIN_PEEL={MIN_PEEL}px) — every separate fiber measured at its thickest point",fill=(230,230,230))
for i,im in enumerate(panels): cv.paste(im,(0,i*(H+6)+22))
cv.save(OUT+"/peel_multifiber.png"); print("saved -> for_katja/peel_multifiber.png")
